...
export default class Layout {
    ...
    private fixAspectRatio(elem: HTMLVideoElement, width: number) {
        const sub = elem.querySelector('.OT_root') as HTMLVideoElement;
        if (sub) {
            // If this is the parent of a subscriber or publisher then we need
            // to force the mutation observer on the publisher or subscriber to
            // trigger to get it to fOT_rootix it's layout
    ...
    private getHeight(elem: HTMLElement) {
        const heightStr = getComputedStyle(elem, null).getPropertyValue("height");
        return heightStr ? parseInt(heightStr, 10) : 0;
    }

    private getWidth(elem: HTMLElement) {
        const widthStr = getComputedStyle(elem, null).getPropertyValue("width");
    ...
    initLayoutContainer(container: HTMLDivElement, opts: LayoutOptions) {
        this.opts = {
